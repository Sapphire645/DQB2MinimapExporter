...
def construct_image(binary_data, tiles, image_width, HD):
    ...
    if valid[3] == 2:
        if valid[4] == 1:
            constructed_image = constructed_image.resize((constructed_image.size[0]*16, constructed_image.size[1]*16), Image.NEAREST)
        try:
            constructed_image = constructed_image.crop((HeresyDictionary[HD][0]-64,HeresyDictionary[HD][1]-64,HeresyDictionary[HD][2]+64,HeresyDictionary[HD][3]+64))
        except:
            valueTemp = BuildertopiaProcess(binary_data)
            constructed_image = constructed_image.crop((valueTemp[0]-64,valueTemp[1]-64,valueTemp[2]+64,valueTemp[3]+64))
...
def process(binary_file_path,image_width, islands,folder_path):
    ...
    for i in range(24):
        if i in islands:
            ...
            if valid[3] == 2:
                constructed_image.save(folder_path+"//"+Name_dictionary_ChunkEditor[i]+".png", 'PNG')
            else:
...
valid = [False,"",2,0,0,False] #Python.... [Valid, Folder_Path, Visible, Hermit_Mask/Chunk_Editor_Mask, chunky_configuration, cmndat]
        
def Text_List_Lan():
    with open("Data//MenuText.txt", 'r',encoding='utf-8') as file:
        lines = file.readlines()
        Text_List = [line.strip() for line in lines]
    while len(Text_List) < 46:
...
Name_dictionary_ChunkEditor = {0:"STGDAT01",1:"STGDAT02",2:"STGDAT03",20:"STGDAT031",21:"STGDAT032",3:"STGDAT04",
                   4:"STGDAT05",7:"STGDAT09",8:"STGDAT10",10:"STGDAT12",11:"STGDAT13",12:"STGDAT14",13:"STGDAT16"}
    ...
    if UserInput[0] == True:
        if button_vars[index].get() != -1:
            if button_vars[index].get() == 0:
                ...
                if (index == 8 or index == 9 or index == 10) and valid[3] > 0:
...
def select_command(upda):
    if UserInput[0] == True:
        for var in button_vars:
            if var.get() != -1:
                var.set(upda)
        if button_vars[8].get()== 1 or button_vars[9].get()== 1 or button_vars[10].get() == 1:
            BuildertopiaLabel.config(fg = "red") #BUILDERTOPIA ERROR
        else:
            BuildertopiaLabel.config(fg = "SystemButtonFace") #BUILDERTOPIA ERROR
...
def chunkEditor_command():
    if UserInput[0] == True:
        ...
        if valid[3] == 2:
            ...
            valid[3] = 2
            heresy_button.config(bg = "SystemButtonFace")
            chunkEditor_button.config(bg = "gold")
            if button_vars[10].get() == 1 or button_vars[8].get() == 1 or button_vars[9].get() == 1:
                BuildertopiaLabel.config(fg = "red") #BUILDERTOPIA ERROR
...
chunkEditor_button = tk.Button(
    options_frame,
    height = 3,
    text=f"{Text_List[43]}\n{Text_List[44]}", #Chunk Editor+\nmask
    command=chunkEditor_command
)

chunkEditor_button.grid(row=0, column=4,rowspan=2, padx=0, pady=0,sticky="ns")
...
chunky_button_1x.grid(row=0, column=5, padx=10, pady=0,sticky="nesw")
...
chunky_button_16x.grid(row=1, column=5, padx=10, pady=0,sticky="nesw")
