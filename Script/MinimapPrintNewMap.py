...
def Text_List_Lan():
    with open("MenuText.txt", 'r',encoding='utf-8') as file:
...
def checkfile(file_path):
    ...
    if list(binary_data[0:8]) == list([252, 255, 190, 62, 180, 255, 127, 192]):
        ...
        if len(binary_data) == 0x55DD3A:
            ...
            if list(binary_data[0:4]) == list([0x61,0x65,0x72,0x43]):
                path_Check.config(text=f"{Text_List[2]} (CMN.DAT)",fg = "green") #Valid file! (Debug save failsafe)
                valid[5] = True
                valid[0] = True
            else:
                valid[5] = False
